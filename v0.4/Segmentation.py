...
def my_app():
    ...
    w_shape=[5, 5, 1, 32]
    b_shape=[32]
    h_conv1=bg.create_conv_layer( x, new_shape, w_shape, b_shape)

    # pooling layer
    ksize = [1, 2, 2, 1],
    strides = [1, 2, 2, 1]
    h_pool1 = bg.max_pool_2x2(h_conv1,ksize,strides)

    # --fully connected layer---------------------------------------------
    w_shape=[13 * 13 * 32, 1024]
    b_shape=[1024]
    flat_shape=[-1, 13 * 13 * 32]
    h_fc1=bg.create_fully_connected_layer(h_pool1, w_shape, b_shape,flat_shape)
