...
class BRATS(object):
    ...
    def make_files_patches(self,data_files_list, label_map_list,patch_size,data_set_type):  # i.e. patch_size=[25,25]
        """
        ...
        # we are taking only two files to test code, in production, it should be full data_files_list etc.
        flist = data_files_list
        mlist = label_map_list
            ...
            for slice_idx in range(slices):  # dimensions are changed and now first dimention gives depth
                ...
                while (cen <= 250):
                    ...
                    try:
                        ...
                        print('some error in finding ',data_set_type,' patch class')
                        exit()
                ...
                for j in range(len(image_patches)):
                    ...
                    try:
                        ...
                        if (data_set_type == 'valid'):
                            np.save('validation_patches/' + fn, image_patches[j])
                        #print(fn)
                    except:
                        print('some error in saving ',data_set_type,' patches')
                        exit()
        print(data_set_type,'  patches created successfully')
    ...
    def get_test_n_train_data(self,train_path,PATHCHES_EXIST):
        self.set_training_path(train_path)
        tfiles = self.read_train_files()

        Tc1,T1,T2,Flair,ground_truth = tfiles[0],tfiles[1],tfiles[2], tfiles[3], tfiles[4]
        if (not PATHCHES_EXIST):
            # You can change the size of training, test and validation set accordingly
            train_set=[Tc1[0],Tc1[1]]
            gt_set=[ground_truth[0],ground_truth[1]]
            self.make_files_patches(train_set, gt_set, [25, 25], 'train')

            test_set=[Tc1[2]]
            gt_set=[ground_truth[2]]
            self.make_files_patches(test_set, gt_set, [25, 25], 'test')

            valid_set=[Tc1[3]]
            gt_set=[ground_truth[3]]
            self.make_files_patches(valid_set, gt_set, [25, 25], 'valid')
        ...
        valid_X, valid_Y = self.read_data_n_labels_4m_patches('validation_patches')
